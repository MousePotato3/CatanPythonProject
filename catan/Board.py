...
class Board:
    ...
    def __init__(self, numPlayers):
        ''' Assign images of settlers tiles to be used in drawing the board '''
        self.desert = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\desert.png")
        self.forest = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\forest.png")
        self.hill = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\hill.png")
        self.mountain = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\mountain.png")
        self.pasture = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\pasture.png")
        self.plain = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\plain.png")
        self.robber = Image.open("C:\\Users\\adhub\\Pictures\\Settlers Tiles\\robber.png")
    ...
    def drawBoard(self):
        ...
        for i in range(len(self.tiles)):
            ...
            d = ImageDraw.Draw(myIcon)
            location = (x, y)
            d.paste(myIcon, location)
        d.show()
    ...
    def getAdjacentHexes(self, point):
        ...
        myInts.append(Board.findHexIndex(self, Point.Point(point.x + self.width, point.y)))
        myInts.append(Board.findHexIndex(self, Point.Point(point.x - self.width, point.y)))
        myInts.append(Board.findHexIndex(self, Point.Point(point.x + self.smallWidth, point.y + self.height)))
        myInts.append(Board.findHexIndex(self, Point.Point(point.x + self.smallWidth, point.y - self.height)))
        myInts.append(Board.findHexIndex(self, Point.Point(point.x + self.smallWidth, point.y + self.height)))
        myInts.append(Board.findHexIndex(self, Point.Point(point.x + self.smallWidth, point.y - self.height)))
        ...
        for i in range(len(playerRoadPoints)):
            ...
            for j in range(len(neighbors)):
                if (self.findRoadIndex(playerRoadPoints[i], neighbors[j], -1) == -1):
                    possibleRoadLocations.append(DoublePoint.DoublePoint(playerRoadPoints[i], neighbors[j]))

        ''' Remove duplicates by converting to a Python set and back to a list '''
        possibleRoadLocations = [*set(possibleRoadLocations)]
